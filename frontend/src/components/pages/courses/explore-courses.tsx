...
export const ExploreCoursesPage = () => {
  ...
  // Remove duplicados por course.id (mantém o primeiro que aparecer)
  const uniqueCourses = Array.from(
    new Map(courses.map((c) => [String(c.id), c])).values(),
  );
    ...
    <div className="min-h-screen">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {defaultTags.map((tag, i) => (
              <Badge
                key={i}
                className={cn(
                  "select-none cursor-pointer bg-secondary text-secondary-foreground",
        ...
        <div className="container flex flex-wrap justify-center gap-5">
          {isLoading &&
            Array.from({ length: 9 }).map((_, i) => (
              <div
                key={i}
                className="w-96 h-64 bg-muted animate-pulse rounded-lg flex items-center justify-center"
              />
            ))}
          ...
          {/*{data?.data?.results.map((course, index) => (
            <CourseItem key={`${course.id}-${index}`} course={course} />
          ))}*/}
          {uniqueCourses.map((course) => (
            <CourseItem key={String(course.id)} course={course} />
          ))}
        </div>
      </div>
      ...
      {(data?.data?.count || 0) > 0 && (
        <Pagination className="mt-8 container">
          <PaginationContent>
            {Array.from({
              length: Math.ceil((data?.data?.count || 0) / 12),
            }).map((_, i) => (
