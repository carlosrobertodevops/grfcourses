...
// Public (AllowAny)
export const getCourses = async (data?: Record<string, any>) => {
  return api<APIGetCoursesResponse>({
    endpoint: "/courses/",
    method: "GET",
    data,
...
// Public (AllowAny) - will 404 if course_id does not exist
export const getCourse = async (courseId: number) => {
  return api<APIGetCourseResponse>({
    endpoint: `/courses/${courseId}/`,
    method: "GET",
...
// Public (AllowAny) - course content (if your backend restricts, flip withAuth to true)
export const getCourseContent = async (courseId: number) => {
  return api<APIGetCourseContentResponse>({
    endpoint: `/courses/${courseId}/content/`,
    method: "GET",
...
export const enrollInCourse = async (courseId: number) => {
  return api<APIEnrollInCourseResponse>({
    endpoint: `/courses/${courseId}/enroll/`,
    method: "POST",
    withAuth: true,
  });
...
export const addCourseReview = async (data: CourseReviewForm) => {
  return api({
    endpoint: `/courses/${data.course_id}/submit_review/`,
    method: "POST",
    data,
    withAuth: true,
  });
